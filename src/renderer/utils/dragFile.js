    ...
    if (files && files.length > 0) {
      ...
      Promise.all(
        fileLists.map(
          (fileList, index) =>
            new Promise((resolve, reject) => {
              ...
              let { message, type: _type } = i18nMsg(paths, repeatList, theCase, type)
              type = _type
              // 添加到vuex中的列表中
        ...
        const messages = res
          .map((message, index) =>
            message.length > 0
              ? `<span style="color: black;font-size: 16px">${i18nTitle(index)}:</span><br />` + message
              : ''
          )
...
function i18nMsg(paths, repeatList, theCase, type) {
  ...
  switch (theCase) {
    case 0:
      ...
      break
    case 1:
      message = paths.length > 0 ? successfulMsg + (hadRepeatFile ? `<br />` + repeatMsg : ``) : ''
      break
    case 2:
...
function i18nOtherFileMsg(notImageCount) {
  const language = store.getters['preferenceStore/preference'].appLanguage
  if (language === 'zh') {
    return notImageCount > 0 ? `${notImageCount}个非图片、视频文件未添加<br />` : ''
  }
  return notImageCount > 0
    ? `${notImageCount} non-picture or non-video file ${notImageCount > 1 ? 's are' : 'is'} not added<br />`
    : ''
}
