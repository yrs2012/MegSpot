  ...
  elementDom.addEventListener(
    ...
    (e) => {
      ...
      if (files && files.length > 0) {
        ...
        let theCase = !(hadRepeatFile || hadOtherTypeFile) ? 0 : paths.length > 0 ? 1 : 2
        let { message, type: _type } = i18nMsg(paths, repeatList, notFolderList, theCase, type)
        message =
          message.length > 0
        ...
        store.dispatch(addToFolderList, paths).then(() => {
          console.log('已添加：', paths, '未添加：', repeatList, '非文件目录', notFolderList)
          Message({
...
function i18nMsg(paths, repeatList, notFolderList, theCase, type) {
  ...
  const folderBeenMsg = (paths) => `${folderMsg(paths)} ${paths.length > 1 ? 'were' : 'was'}`
  const successfulMsg =
    language === 'zh'
  ...
  const repeatMsg =
    language === 'zh'
      ...
      : `${repeatList.length} ${folderMsg(repeatList)} that had been added ${getS(repeatList.length)} not added`
  const otherFileMsg =
    language === 'zh'
  ...
  switch (theCase) {
    ...
    case 1:
      message =
        successfulMsg + (hadRepeatFile ? '<br />' + repeatMsg : ``) + (hadOtherTypeFile ? `<br />` + otherFileMsg : '')
      break
    case 2:
      message =
        (hadRepeatFile ? repeatMsg : ``) + (hadOtherTypeFile ? '<br />' : '') + (hadOtherTypeFile ? otherFileMsg : '')
      break
  }
