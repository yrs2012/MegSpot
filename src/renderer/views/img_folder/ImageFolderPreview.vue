  ...
  <div class="preview" ref="preview" flex="dir:top">
    ...
    <div flex-box="1" class="preview-content" v-show="showType === 'list'">
      <FileTable
        ...
        :selectFolderMode="selectFolderMode"
        :selectFolderSortChanged="handleSelectFolderSortChange"
      ></FileTable>
    </div>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters(['imageList', 'imageFolders', 'imageConfig', 'folderFileListMap']),
  ...
  methods: {
    ...
    handleSelectAll({ checked }) {
      if (this.allSelectd) {
        this.removeImages(this.thumbnailList.filter((item) => item.isFile).map((item) => item.path))
      } else {
        this.addImages(this.thumbnailList.filter((item) => item.isFile).map((item) => item.path))
      }
    },
    ...
    handleSelectFolderSortChange(sortedList) {
      if (this.selectFolderMode) {
        // if currentPath in selectImage
        //    replace imagelist in folderFileListMap
      }
    },
