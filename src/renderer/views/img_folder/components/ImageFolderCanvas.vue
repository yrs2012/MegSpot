  ...
  <div :class="['image-canvas', { selected: selected }]" @click.stop>
    ...
    <div ref="header" :class="['header', selected ? 'selected-item' : '']" flex="main:justify cross:center">
      ...
      <div v-show="videoSliderVisible" class="progress-container" flex="main:center cross:center" flex-box="1">
        <video-slider
          :value="playIndex / frameChangeRate"
          :max="(fileList.length-1) / frameChangeRate"
          :interval="1"
      ...
      <div class="header-right" flex="main:right cross:center" flex-box="0">
        <div v-show="videoSliderVisible" class="video-tool" flex="cross:center">
          <input :value="(playIndex / frameChangeRate).toFixed(2)" @change="changeVideoTime" class="time-input" />
          <FrameSetting
            :path="path"
            :frameRate.sync="frameRate"
            :frameCount.sync="fileList.length"
...
export default {
  ...
  data() {
    return {
      ...
      scheduleCanvasActions: [
        ...
        {
          event: 'imageFolder_frameSteps',
          action: 'handleFrameSteps'
        },
        {
          event: 'changeImageFolderSliderVisible',
          action: 'changeVideoSliderVisible'
        },
        {
          event: CONSTANTS.BUS_IMAGE_FOLDER_COMPARE_ACTION,
          action: 'executeAction'
        },
        {
          event: 'imageFolderResetTime',
  ...
  computed: {
    ...
    curPlayPath() {
      if(this.playIndex >= this.fileList.length) {
        return ""
      }
      return this.fileList[this.playIndex]
    },
    previousFrameAvailable() {
      return this.paused && this.playIndex > 0
    },
    nextFrameAvailable() {
      return this.paused && this.playIndex < this.fileList.length
    },
    displayedFrames() {
      return this.frameRate
        ? Math.min(this.playIndex + 1, this.fileList.length)
        : 0
    },
  ...
  methods: {
    ...
    resolvePath() {
      return new Promise((resolve) => {
        ...
        if (this.snapshotMode) {
          ...
          resolve(getImageUrlSyncNoCache(playPath)) //        'C:/Demo/1-1%20-%20副本.jpg'
        }
      })
    },
    async initImage(initPosition = true) {
      this.loading = true
      var playPath = this.curPlayPath
      if(playPath.length == 0) {
        return
      }

      if (/tiff?$/.test(playPath)) {
        const file = await fse.readFile(playPath)
    ...
    loadImageSync() {
      ...
      // } else 
      {
        ...
        image.onload = async () => {
          ...
          // this.loading = false
          // this.reDraw(initPosition)
          var newBitMap = await createImageBitmap(image)
          this.loading = false
          if(this.bitMap) {
            let { x, y, width, height } = this.getImageInitPos(this.canvas, this.bitMap)
            
            this.cs.clearRect(0, 0, this.canvas.width, this.canvas.height)
            this.cs.drawImage(newBitMap, x, y, width, height)
        ...
        image.src = getImageUrlSyncNoCache(playPath) 
      }
    },
    initCanvas() {
    ...
    handleVideoSliderVisible(status) {
      this.$bus.$emit('changeImageFolderSliderVisible', status ?? !this.videoSliderVisible)
    },
    ...
    changeVideoTime(event) {
      ...
      this.playIndex = Math.round(currentTime)
      this.initImage(false)
    },
    changeFrame(step = 0) {
      let nextIndex = this.playIndex + step
      if (nextIndex < 0 || nextIndex >= this.fileList.length) {
        console.log('invalid nextFrame')
        return false
      }
      // if (this.paused) {
        this.changeVideoTime(nextIndex)
      // }
      return true
    },
    handleFrameSteps({ name, data }) { // // 逐帧对比
      !this.videoSliderVisible && this.changeVideoSliderVisible({ value: true })
      // // 存在选中使只使选中视频进行逐帧操作
      if (this.selectedId && this.selectedId !== this.curPlayPath) return
    ...
    executeAction(action) {
      switch (action) {
        ...
        case CONSTANTS.IMAGE_FOLDER_PLAY_STATUS_START:
          ...
          if (this.playIndex >= this.fileList.length-1) {
            // this.offset = 0
            // this.currentTime = 0
          ...
          // },1000)
          }, 1000/this.frameRate/this.imageConfig.speed)
    ...
    handleVideoResetTime() {
      console.info("-----handleVideoResetTime----")
    }
  }
}
</script>
