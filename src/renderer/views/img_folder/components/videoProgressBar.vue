...
export default {
  ...
  mounted() {
    ...
    this.$bus.$on(CONSTANTS.BUS_IMAGE_FOLDER_COMPARE_ACTION, this.executeAction)
  },
  beforeDestroy() {
    this.timer && clearTimeout(this.timer)
    this.$bus.$off('videoLoaded', this.handleVideoLoaded)
    this.$bus.$off(CONSTANTS.BUS_IMAGE_FOLDER_COMPARE_ACTION, this.executeAction)
  },
  methods: {
    ...
    getTime() {
      ...
      if (this.max > 0 && this.currentTime >= this.max) {
        this.$bus.$emit('imageFolderResetTime')
        this.setImageConfig({ currentTime: 0 })
      }
      this.timer = setTimeout(() => {
        this.setImageConfig({ currentTime: this.currentTime + 1 })
        this.getTime()
      }, 1000)
    },
    ...
    handleInput(value) {
      if (value !== this.lastValue) {
        ...
        if (this.currentTime != value) {
          this.setImageConfig({ currentTime: value })
  ...
  computed: {
    ...
    currentTime() {
      return this.imageConfig.currentTime
    }
  }
}
</script>
