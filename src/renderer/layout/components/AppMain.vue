...
export default {
  ...
  beforeCreate: async function () {
    ...
    if (unExistPath.length) {
      this.$message({
        type: 'warning',
        message: `Some path arg no exist and is ignored.${JSON.stringify(unExistPath)}`
      })
    }
    // 识别模式
    let mode
    ...
    if (checkedPath.length === 0) {
      ...
      if (checkedPath.length > 1) {
        this.$message({
          type: 'warning',
          message: `Jump to file select page.other args is ignored:${JSON.stringify(checkedPath.slice(1))}`
        })
      }
    } else {
      // 其他的作为对比文件增加到对比队列中
